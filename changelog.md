...
- [ Fixed ] Copying a built-in Redshift LUT from Cinema 4D or Houdini into After Effects now **selects it in the LUT dropdown** instead of storing it as a manual file path — so you can still change it from the menu afterwards. The bridge now recognises every Redshift install location, including a standalone Redshift folder and whatever your Redshift is actually using, when working out the LUT's name (macOS and Windows)
